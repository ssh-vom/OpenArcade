...
def aggregator_process(
    ...
    def check_pairing_change() -> tuple[bool, bool]:
        ...
        try:
            ...
            # Check if pairing state file was modified
            try:
    ...
    async def run() -> None:
        ...
        def get_pairing_status() -> dict[str, Any]:
            """Return current pairing and scanner state for IPC."""
        ...
        def should_scan() -> bool:
            ...
            # Pairing mode must be enabled to scan
        ...
        # Only start scanner if pairing is enabled
        if pairing_enabled:
            await ensure_scanner_running()
        ...
        try:
            while not stop_event.is_set():
                ...
                if pairing_changed:
                    if not now_enabled:
                        # Pairing turned OFF: stop scanner, clear pending work
                        await stop_scanner()
                        pending_connects.clear()
                        retry_after.clear()
                        # Note: connected_clients remain connected
                        logger.info(
                            "Pairing disabled - scanner stopped, pending connects cleared. "
                            f"Active connections preserved: {len(connected_clients)}"
                        )
                    else:
                        # Pairing turned ON: extend scan window and allow scanning
                        extend_scan_window()
