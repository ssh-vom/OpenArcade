...
- `openarcade-display.service`: SPI SSD1306 status display for module count and Pi temperature
...
- enables the `dwc2` overlay, enables SPI, and loads `libcomposite`

If the installer enables `dwc2` or SPI for the first time, reboot once after install.
